...
class Dbupdate:
    @staticmethod
    def check_value(dict, key, default):
    ...
    def update(cls):
        ...
        for product in all_products:
            ...
            if int(result['status']):
                ...
                new_product = Product(
                    id = product.id,
                    name=cls.check_value(result, 'product_name', product.name),
                    brands=cls.check_value(result, 'brands', product.brands),
                    tags=cls.check_value(result, 'categories_tags',
                                         product.tags),
                    ingredients=cls.check_value(result, 'ingredients_text_fr',
                                                product.ingredients),
                    ...
                    nutriscore=cls.check_value(result, 'nutriscore_grade',
                                               product.nutriscore),
                    labels=cls.check_value(result, 'labels', product.labels),
                    stores=cls.check_value(result, 'stores_tags',
                                           product.stores),
                    link=cls.check_value(result, 'url',
                                         product.link),
                    compared_to=cls.check_value(result, 'categories_hierarchy',
                                                product.compared_to),
